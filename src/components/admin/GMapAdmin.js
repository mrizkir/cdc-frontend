...
const GMap = ({ koordinat }) => {
  ...
  if (koordinat.length > 0) {
    for (var i = 0; i < koordinat.length; i++) {
      marker.push({
        nama: koordinat[i].name,
        status: koordinat[i].nama_status,
        foto: koordinat[i].foto,
        usia: koordinat[i].usia,
        id: koordinat[i].id,
        lat: parseFloat(`${koordinat[i].lat}`),
        lng: parseFloat(`${koordinat[i].lng}`),
        icon: pilihIcon(koordinat[i].status_pasien),
      });
    }
    marker.push(
      {
          ...
          content: `<div id="test" className="card mb-3 text-center" >
                                ...
                                <div className="card-body">
                                   ...
                                   <p  style="margin:2px;font-weight:bold !important;text-transform:uppercase"> NAMA: ${x.nama} </p>
                                    ...
                                    <br/>
                                    <a class="btn btn-sm btn-info" href="/admin/detailpasien/${x.id}">Lihat Detail</a>
                                </div>
