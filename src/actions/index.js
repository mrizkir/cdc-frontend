...
export const getUser = () => async (dispatch) => {
  ...
  var data = null;
  if (token) {
    await Api.get("/auth/me", { headers: { Authorization: token } })
      .then((response) => {
        data = response.data;
...
export const getPasien = () => async (dispatch) => {
  ...
  var data = [];
  if (token) {
    await Api.get("/setting/userspasien", { headers: { Authorization: token } })
      .then((response) => {
        data = response.data;
      })
      .catch((error) => {});
...
export const getDetailPasien = (id) => async (dispatch) => {
  ...
  var data = null;
  if (token) {
    await Api.get(`/setting/userspasien/${id}`, {
      headers: { Authorization: token },
    })
      .then(async (response) => {
        data = response.data;
...
export const ubahStatusPasien = (id, formValues) => async (dispatch) => {
  ...
  if (token) {
    await Api.post(
      `/setting/userspasien/updatestatus/${id}`,
      { _method: "PUT", ...formValues },
      { headers: { Authorization: token } }
    ).then(
      (res) => {},
...
export const uploadFoto = (id, formValues) => async (dispatch) => {
  ...
  if (token) {
    await Api.post(`/setting/users/uploadfoto/${id}`, formData, {
...
export const getLaporan = () => async (dispatch) => {
  const token = "Bearer " + localStorage.token;
